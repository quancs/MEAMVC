...
////
// btree methods
template <typename P> btree<P>::btree(const key_compare &comp, const allocator_type &alloc)
    : key_compare(comp),
      root_(alloc, NULL) {
}

template <typename P> btree<P>::btree(const self_type &x)
...
template <typename P>
typename btree<P>::reverse_iterator btree<P>::erase(reverse_iterator iter) {
    return btree<P>::reverse_iterator(erase((++iter).base()));
}
